...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      // Cap maximum internal resolution to significantly improve performance 
      // especially when the container is extremely large (e.g. 2400x2400)
      const MAX_PIXELS = 1000;
      let effectiveDpr = dpr;
      if (Math.max(w, h) * dpr > MAX_PIXELS) {
        effectiveDpr = MAX_PIXELS / Math.max(w, h);
      }
